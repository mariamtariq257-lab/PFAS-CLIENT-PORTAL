...
// ── Shared design tokens (inline, so they apply regardless of external CSS) ────
const CARD = {
  background: "#fff",
        ...
        <div className="pin-inputs">
          {digits.map((d, i) => (
            <input
              ...
              onChange={(e) => handleKey(i, e)} onKeyDown={(e) => handleKeyDown(i, e)} autoComplete="off"
            />
          ))}
        </div>
        {error && <div className="pin-error-msg">Incorrect PIN. Please try again.</div>}
        <button className="admin-modal-cancel" onClick={onClose}>Cancel</button>
        <div className="admin-modal-hint">Contact your PFAS system administrator if you've forgotten your PIN.</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="admin-picker-overlay">
      ...
      <div className="admin-picker-body">
        <div className="admin-picker-meta">{ADMIN_CLIENT_LIST.length} clients · {ALL_PROJECTS.length} projects</div>

        <div className="admin-search-wrap">
          <span className="admin-search-icon">🔍</span>
          <input className="admin-search-input" type="text" placeholder="Search clients or projects…"
            value={search} onChange={e => setSearch(e.target.value)} autoFocus />
          {search && <button className="admin-search-clear" onClick={() => setSearch("")}>✕</button>}
        </div>

        <div className="admin-client-grid">
          {filtered.length === 0 && <div className="admin-no-results">No clients match "{search}"</div>}
            ...
            return (
              <div className="admin-client-card" key={ci}>
                <div className="admin-card-header">
                  <div className="admin-card-avatar" style={{ background: dc.bg, color: dc.color }}>{initials}</div>
                  <div>
                    <div className="admin-card-name">{client.name}</div>
                    <div className="admin-card-count">{client.projects.length} project{client.projects.length > 1 ? "s" : ""}</div>
                  </div>
                </div>
                <div className="admin-project-list">
                  {client.projects.map((p, pi) => (
                    <button key={pi} className="admin-project-btn" onClick={() => onSelect(p.slug, client.name, client.projects)}>
  ...
  return (
    <div className="login-overlay">
      <div className="login-card">
        ...
        <div className="dev-banner">🛠 Dev mode — Auth0 bypassed on localhost.<br />Select a client to preview their portal.</div>
        <select className="dev-select" value={selected} onChange={e => setSelected(e.target.value)}>
          {ALL_PROJECTS.map(p => <option key={p.slug} value={p.slug}>{p.name}</option>)}
        </select>
        <button className="login-btn" onClick={() => onSelect(selected, ALL_PROJECTS.find(p => p.slug === selected)?.name)}>
          Open Portal →
        </button>
    ...
    <div className="login-overlay">
      <div className="login-card">
        ...
        <div style={{ textAlign: "center", padding: "24px 0 8px" }}>
          <button className="login-btn" onClick={onLogin} style={{ marginTop: 0 }}>Sign In Securely</button>
        </div>
        ...
        <div className="login-footer">
          ...
          <button className="admin-link-btn" onClick={onAdminClick} title="Admin access">🔐 Admin</button>
        </div>
      </div>
    </div>
  );
...
function LoadingSkeleton() {
  return (
    <div style={{ padding: "8px 0", maxWidth: 1480, margin: "0 auto" }}>
      <div style={{ background: "#E2E8F0", borderRadius: 16, height: 120, marginBottom: 18, animation: "shimmer 1.5s infinite" }} />
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 18 }}>
        {[1,2,3,4].map(i => <div key={i} style={{ background: "#E2E8F0", borderRadius: 14, height: 92, animation: "shimmer 1.5s infinite" }} />)}
      </div>
  ...
  return (
    <div ref={wrapRef} style={{ position: "relative", display: "inline-block" }}>
      <button onClick={() => setOpen(o => !o)}
        style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 12px", background: "rgba(255,255,255,0.12)", color: "#fff", border: "1px solid rgba(255,255,255,0.2)", borderRadius: 16, fontSize: 12, fontWeight: 600, cursor: "pointer", maxWidth: 280 }}>
        <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{current.label}</span>
        <span style={{ fontSize: 10, opacity: 0.8 }}>▾</span>
      </button>
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 6px)", right: 0, background: "#fff", border: "1px solid #E2E8F0", borderRadius: 12, boxShadow: "0 10px 30px rgba(0,0,0,0.12)", padding: 6, zIndex: 100, minWidth: 280, maxHeight: 360, overflowY: "auto" }}>
          <div style={{ fontSize: 10, letterSpacing: 1.2, fontWeight: 700, color: "#94A3B8", padding: "8px 12px 4px", textTransform: "uppercase" }}>Switch Project</div>
          {projects.map(p => {
            const isActive = p.slug === currentSlug;
            return (
              <button key={p.slug} onClick={() => { onChange(p.slug); setOpen(false); }}
                style={{ display: "block", width: "100%", textAlign: "left", padding: "10px 12px", background: isActive ? "#F1F5F9" : "transparent", border: "none", borderRadius: 8, fontSize: 13, fontWeight: isActive ? 600 : 500, color: isActive ? "#1F3A5F" : "#334155", cursor: "pointer" }}
  ...
  return (
    <div className="topbar">
      ...
      <div className="topbar-right">
        {isAdmin && <span className="admin-topbar-badge">🔐 Admin</span>}
        {showSwitcher && <ProjectSwitcher projects={projects} currentSlug={currentSlug} onChange={onProjectChange} />}
        <span className="live-badge"><span className="live-dot" />{lastUpdated ? `Updated ${lastUpdated}` : "LIVE · ClickUp"}</span>
        <span className="user-chip"><span className="av">{initial}</span><span>{userName}</span></span>
        {isAdmin
    ...
    <>
      {sorted.map((ph, i) => (
        <div className="phase-row" key={i}>
          ...
          <div className={`phase-bar b-${ph.status}`}><span style={{ width: `${ph.pct}%` }} /></div>
        </div>
      ))}
    </>
  );
...
// Avatar palette — deterministic color per member by index
const AV_COLORS = [
  "linear-gradient(135deg,#1F3A5F,#2C4F7C)",
  "linear-gradient(135deg,#166534,#22A35A)",
  "linear-gradient(135deg,#92400E,#C9821F)",
  "linear-gradient(135deg,#6B21A8,#9333EA)",
  "linear-gradient(135deg,#155E75,#0E9FB8)",
  "linear-gradient(135deg,#9B2C2C,#D9534F)",
];
...
function TeamGrid({ team }) {
  ...
  return (
    <div className="team-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(270px, 1fr))", gap: 14 }}>
      {team.map((m, i) => {
        ...
        return (
          <div className="member-card" key={i}
            style={{ display: "flex", flexDirection: "column", gap: 12, padding: 18, background: "#fff", border: "1px solid #E8ECF2", borderRadius: 14, minWidth: 0, boxShadow: "0 1px 2px rgba(16,24,40,0.04)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, minWidth: 0 }}>
              <div style={{ flexShrink: 0, width: 46, height: 46, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 700, fontSize: 15, background: avatarBg, boxShadow: "0 2px 6px rgba(31,58,95,0.25)" }}>
                {initials.toUpperCase()}
              </div>
              <div style={{ minWidth: 0, flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: 15, color: "#0F1E33", lineHeight: 1.25 }}>{m.name}</div>
                <div style={{ fontSize: 12, color: "#64748B", lineHeight: 1.35, marginTop: 3 }}>{designation}</div>
              </div>
            </div>

            <div style={{ display: "flex", flexDirection: "column", gap: 6, paddingTop: 12, borderTop: "1px solid #F1F5F9" }}>
              {emailDisplay && (
            ...
            {waHref && (
              <a href={waHref} target="_blank" rel="noreferrer"
                style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 7, padding: "8px 12px", background: "#F0FDF4", color: "#166534", fontSize: 12.5, fontWeight: 600, borderRadius: 9, textDecoration: "none", border: "1px solid #BBF7D0", transition: "background 0.15s" }}>
...
function TeamsPanel({ project }) {
  ...
  useEffect(() => {
    if (chatBodyRef.current && msgs.length > 0) chatBodyRef.current.scrollTop = chatBodyRef.current.scrollHeight;
  }, [msgs]);

  if (!hasLiveChannel) {
    return (
      <div className="chat-card teams-panel" style={{ ...CARD, padding: 0, overflow: "hidden" }}>
        <div className="chat-header">
          ...
          <div>
            ...
            <div className="chat-status-pending"><span className="pending-dot" /> Connecting to Microsoft Teams…</div>
          </div>
        </div>
  ...
  return (
    <div className="chat-card teams-panel" style={{ ...CARD, padding: 0, overflow: "hidden" }}>
      <div className="chat-header">
        ...
        <a href={project.teamsChannel || "#"} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: "rgba(255,255,255,0.75)", textDecoration: "none", flexShrink: 0 }}>Open ↗</a>
      </div>
...
function DocumentsSection({ project }) {
  ...
  return (
    <SectionCard title="Project Documents">
      <div className="docs-recent-label" style={{ fontSize: 11, fontWeight: 600, letterSpacing: 0.4, textTransform: "uppercase", color: "#94A3B8", marginBottom: 10 }}>Recently Uploaded</div>
      <div className="docs-file-list" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {RECENT_FILES_PLACEHOLDER.map((f, i) => {
          ...
          return (
            <a key={i} className="doc-file-row" href={uploadUrl} target="_blank" rel="noreferrer"
              style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", borderRadius: 10, border: "1px solid #F1F5F9", textDecoration: "none", transition: "background 0.15s, border-color 0.15s" }}>
...
function ActionsGrid({ project }) {
  ...
  return (
    <div className="actions-grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12 }}>
      {actions.map((a, i) => (
        <a key={i} className="action-btn" href={a.href} target="_blank" rel="noreferrer"
          style={{ display: "flex", alignItems: "center", gap: 13, padding: 15, borderRadius: 12, border: "1px solid #EEF1F6", textDecoration: "none", background: "#fff", transition: "transform 0.12s, box-shadow 0.12s, border-color 0.12s" }}
          onMouseEnter={(e) => { e.currentTarget.style.boxShadow = "0 4px 14px rgba(16,24,40,0.08)"; e.currentTarget.style.borderColor = "#D9E1EC"; e.currentTarget.style.transform = "translateY(-1px)"; }}
          onMouseLeave={(e) => { e.currentTarget.style.boxShadow = "none"; e.currentTarget.style.borderColor = "#EEF1F6"; e.currentTarget.style.transform = "none"; }}>
...
export default function ClientPortal() {
  ...
  const handleAdminPinSuccess = () => { setShowPinModal(false); setIsAdmin(true); setAuthState("adminpicker"); };
  ...
  const handleAdminSwitch = () => { setAuthState("adminpicker"); setProject(null); setSlug(""); };
  const handleAdminExit = () => { setIsAdmin(false); setAuthState("login"); setProject(null); setSlug(""); setUserName(""); };
  ...
  if (authState === "devpicker") return <>{headAndCss}<DevPicker onSelect={handleDevSelect} /></>;
  ...
  if (authState === "login") {
    return (
      <>
        ...
        {showPinModal && <AdminPinModal onSuccess={handleAdminPinSuccess} onClose={() => setShowPinModal(false)} />}
      </>
    );
  }

  if (authState === "adminpicker") {
    return <>{headAndCss}<AdminClientPicker onSelect={handleAdminSelect} onBack={handleAdminExit} /></>;
  }
  ...
  return (
    <>
      {headAndCss}
      <TopBar
        userName={userName} onLogout={handleLogout} lastUpdated={project?.lastUpdated}
        isDev={isDev} onSwitchDev={handleDevSwitch}
        projects={allowedProjects} currentSlug={projectSlug} onProjectChange={handleProjectChange}
        isAdmin={isAdmin} onAdminSwitch={handleAdminSwitch}
      />
      <div className="container">
        <div className="hero" style={{ position: "relative", overflow: "hidden", borderRadius: 18, padding: "28px 30px", marginBottom: 20, background: "linear-gradient(120deg,#16294A 0%,#1F3A5F 55%,#2C4F7C 100%)", boxShadow: "0 6px 24px rgba(22,41,74,0.28)" }}>
          ...
          <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: 1.5, textTransform: "uppercase", color: "rgba(255,255,255,0.6)" }}>Project Portfolio Overview</div>
          <div style={{ fontSize: 24, fontWeight: 800, color: "#fff", margin: "8px 0 6px", lineHeight: 1.2 }}>Welcome to your PFAS engagement portal</div>
          <div style={{ fontSize: 13.5, color: "rgba(255,255,255,0.78)", maxWidth: 640, lineHeight: 1.5 }}>Track project progress, contact your advisory team directly, access shared documents and meeting minutes, and schedule meetings — all in one place.</div>
        </div>

        {dataLoading && <LoadingSkeleton />}
        {dataError && (
          <div style={{ background: "#FECACA", border: "1px solid #F87171", borderRadius: 12, padding: "16px 20px", color: "#9B2C2C", marginBottom: 20 }}>⚠ {dataError}</div>
        )}
